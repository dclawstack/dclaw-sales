...
### How do I update DClaw Sales?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp sales --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Sales without Kubernetes?

Yes, for local development:

```bash
cd dclaw-sales/frontend && npm run dev
cd dclaw-sales/backend && uvicorn dclaw_sales.main:app --reload
```
